...
def find_max_following_all_changes(instructions,values,changes_function,assess_function):
...
test_instructions_raw =["b inc 5 if a > 1",
"a inc 1 if b < 5",
"c dec -10 if a >= 1",
"c inc -20 if c == 10"]
...
test_instructions, test_vals = clean_data_fn(test_instructions_raw)
final_vals = make_all_changes(test_instructions, test_vals,make_changes,assess_condition)

assert final_vals['c'] == -10

problem_instructions, problem_vals = clean_data_fn(problem_data_raw)

final_vals = make_all_changes(problem_instructions, problem_vals,make_changes,assess_condition)

print('8.a',find_max_dict_value(final_vals))

problem_instructions, problem_vals = clean_data_fn(problem_data_raw)

highest_ever = find_max_following_all_changes(problem_instructions, problem_vals,make_changes,assess_condition)
